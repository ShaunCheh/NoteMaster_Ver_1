---
name: SR1判题分阶段
overview: 基于现有 `quarterNoteSequence`、`ExerciseScene` 和 `Settings` 架构，先把 sequence 判题语义从硬编码 `PitchClass` 提升为可配置 `answerPolicy`，再以新 `staffToPiano` 组合和独立 `SR-1` mode 接入 UI。SR-1 当前固定按音名判题，未来 SR-2 只切换到 `exactNote`，不再重写 router、trainer 或 scene。
todos:
...
- 具体动作：给 `TrainerSequenceConfiguration` 增加 `answerPolicy`；给 `QuarterNoteSequenceSpec` 同步增加 `answerPolicy`；补齐 `TrainerSequenceConfiguration.init(quarterNoteSequenceSpec:)` 与 `quarterNoteSequenceSpec` 的双向映射；在 `QuarterNoteSequenceEvaluation` 里预埋 `comparisonPolicy` 和 `expectedNotePitch` 只读投影。
...
- 具体动作：给 `ExerciseAnswerPayload` 增加 `.notePitch(NotePitch)`；引入 `ResolvedSequenceAnswer`，至少保留 `pitchClass`、`notePitch?`、`surfaceID`；让 sequence 路由不再返回裸 `pitchClass`，而是返回统一的 `ResolvedSequenceAnswer`；在 trainer 侧新增 `resolvedSequenceAnswer(...)` 辅助函数，统一从 `pitchClass` / `fretboardCell` / 未来的 `piano` 输入恢复 sequence 需要的语义。
