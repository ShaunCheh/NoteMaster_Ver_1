        ...
        if RuntimeSmokeScenario.shouldRunSR0NoteStripAnswerSmoke {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) { [weak self] in
                guard
                    ...
                    let summary =
                        "[RuntimeSmoke][iOS] FAIL scenario=sr1_piano_answer reason=missing_window_or_view_controller"
                    print(summary)
                    fatalError(summary)
                }
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) { [weak self] in
                guard
                    ...
                    let summary =
                        "[RuntimeSmoke][iOS] FAIL scenario=fr0-circle-feedback reason=missing_window_or_view_controller"
                    print(summary)
                    fatalError(summary)
                }
